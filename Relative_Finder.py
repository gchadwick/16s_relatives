...
os.chdir("/Users/g/Desktop/Methanomicrobia/") #Set directory to one containing fasta

files = 'GoM-Arch87.fasta'
file_name = files.split['.'][0].split['_renam'][0]
...
for sequence in sequence_list:
	...
	split_holder=sequence.format("fasta").split('\t')[1].split(' ')
	bp = int(split_holder[0])
	if bp >= 700:
		sequence_relatives = NCBIWWW.qblast("blastn", "nr", sequence.seq, hitlist_size = 1000, perc_ident = 90, megablast='TRUE')
			...
			for hsp in alignment.hsps:
				...
				if length >= 600:
					...
					if index == -1:
						...
						counter2 += 1
		save_1_report.write(sequence.id)
